...
taken. The second filter is not optional in either case: differentiation amplifies the high end
and integration accumulates the low end, so a signal band-limited only before the operation is not
band-limited after it.
...
quantity — in one large corpus, how much a person moved predicted only about a quarter of how
...
Directions are not ordinary numbers — 359° and 1° are two degrees apart, not 358 — so they need
their own mean and their own tests.

```python
mm.circ_mean(angles)
mm.rayleigh(angles)              # is there a preferred direction at all?
mm.rayleigh_axial(angles)        # ...for an axis, where 0 and 180 are the same
mm.axial_dispersion(angles)
mm.circ_corr(a, b)               # circular-circular
mm.circ_corr_linear(a, x)        # circular-linear
mm.vtest(angles, mu)             # a preferred direction at a *predicted* angle
```

Postural sway is usually **axial** rather than directional — a body rocking along a line visits
...
fluctuations are organised across timescales — a question that turns out to distinguish standing
bodies more sharply than amplitude does.
...
series is persistent — a deviation tends to be followed by more of the same.
...
!!! warning "A wide spectrum is more often a preprocessing fault than a finding"
    ...
    has produced widths up to 6.6 where the plausible range is around 1 — the interpolation creates
    structure between samples and the method reads it as real.

### Stabilogram diffusion analysis

```python
mm.sda(xy, fs)     # -> Hs, Hl, critical time and displacement
```
...
```python
mm.sampen(x, m=2, r=None)     # sample entropy
mm.apen(x, m=2, r=None)       # approximate entropy
```
...
```python
mm.rqa(x, ...)                 # determinism, laminarity, entropy, trapping time
mm.embed(x, m, tau)
mm.ami(x); mm.first_ami_minimum(x)
```
...
This implementation solves the threshold **per plot so that a fixed fraction of pairs count as
recurrent**. With a fixed absolute threshold, determinism partly measures how tightly packed the
trajectory is, and two recordings of different amplitude are then not comparable — a trap worth
knowing about when reading the older literature.
...
it differs from what a simpler process would give. That is what surrogate testing is for.

```python
mm.iaaft(x)                       # amplitude distribution + power spectrum preserved
mm.phase_surrogate(x)             # power spectrum preserved, phases randomised
mm.circular_shift_surrogate(x)    # within-series structure preserved, alignment destroyed
mm.surrogate_test(x, statistic, kind="iaaft")
```
...
```python
mm.dcca(a, b, scales)             # correlation per timescale
mm.plv(a, b, fs, band)            # phase locking
...
It takes event times from outside — musical onsets, annotated moments, stimulus boundaries — so
the events can come from an analysis of the stimulus rather than from the movement itself.

It counts signs rather than sizes deliberately. Participants in group recordings wear different
...
Most of these methods have parameters that change the answer. Where this package picks a default,
...
And one that is deliberately left to you: the **integration rule** for turning acceleration into
speed. Rectangle and trapezoid differ by about 0.26 per cent systematically, the rectangle sum
lagging by half a sample. Neither is universally right; pick one and record it.
